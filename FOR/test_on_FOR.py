...
# -------------------- Dataset: FOR (Face Forensics) --------------------
class FORDataset(Dataset):
    def __init__(self, for_root, split='testing', sample_rate=16000, n_mels=128, n_fft=1024, hop_length=160):
        self.items = []
        if split == 'all':
            ...
            splits = [split]
        for s in splits:
            split_dir = os.path.join(for_root, s)
...
def eval_at_threshold(model, loader, device, threshold=0.5):
    ...
    y_true, y_score = [], []
    with torch.no_grad():
        for mel, label in tqdm(loader, desc="Evaluating FOR"):
            mel = mel.to(device)
            out = model(mel)
...
def main():
    ...
    parser.add_argument('--for_root', type=str, default=r"c:\Users\Shivaay Dhondiyal\Desktop\shivaay\coding\2_projects\9_deepfake_paper\FOR-2SEC", help='FOR dataset root')
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--out_log', type=str, default='FOR_eval_results.txt')
    ...
    dataset = FORDataset(args.for_root, split='testing')
    ...
    print(f"FOR Eval Done: Acc={acc:.4f} EER={eer:.4f} AUC={auc:.4f}")
